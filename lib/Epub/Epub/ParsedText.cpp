...
bool ParsedText::layoutAndExtractLines(const GfxRenderer& renderer, const int fontId, const uint16_t viewportWidth,
  ...
  if (lineCount > 0) {
    ...
    const size_t linkErase = std::min(consumed, wordLinkIds.size());
    wordLinkIds.erase(wordLinkIds.begin(), wordLinkIds.begin() + linkErase);
...
void ParsedText::extractLine(const size_t lineIndex, const size_t lineBreak, const size_t lastBreakAt,
  ...
  for (size_t i = 0; i < lineWordCount && wordLinkIds.size() == words.size(); i++) {
    ...
    const auto style = willReorder ? reorderedStylesScratch[i] : lineWordStyles[i];
    const int topLift = (style & EpdFontFamily::SUP) != 0 ? renderer.getFontAscenderSize(fontId) * 2 / 5 : 0;
