...
- `section.bin`: **per-flavor** — Latin builds **Version 58**, Chinese builds
  (`ENABLE_CHINESE_VERSION`) **Version 59**. Versions 58/59 append internal-link
  rectangles to each serialized page for touch navigation. The same built-in font
  IDs resolve to different font data and metrics in the two firmware flavors, so
  independent counters prevent a reflash from reusing pagination generated by
  the other flavor.
